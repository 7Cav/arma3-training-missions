		...
		if (typeOf player == "rhsusf_usmc_marpat_wd_officer") then {
			{			
				...
				{				
					...
					// - the assigned team color R,G,B at a moderate 0.5 transparency (Civs 128,128,0 East 255,0,0 West 0,0,255)
					// - the unit's X,Y height of 1 meter
					// - normal width
					// - normal height
					// - no rotation
					// - player's in-game name under the icon
					drawIcon3D[getText (configFile >> "CfgVehicles" >> typeOf _x >> "icon"),[255,0,0,0.5],[getPos _x select 0,getPos _x select 1,(getPos _x select 2)+3],1,1,0, str (floor(_x distance player))];
				};			
			} foreach allUnits;
		};
	}];