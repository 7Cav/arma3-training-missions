		...
		if (typeOf player == "rhsusf_usmc_marpat_wd_crewman") then {
			{			
				...
				{				
					...
					drawIcon3D[getText (configFile >> "CfgVehicles" >> typeOf _x >> "icon"),[128,0,128,0.5],[getPos _x select 0,getPos _x select 1,(getPos _x select 2)+3],1,1,0, str (floor(_x distance player))];
				};			
			} foreach allUnits;
		};
	}];